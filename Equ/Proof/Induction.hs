...
createIndHypothesis :: Relation -> PE.Focus -> PE.Focus -> PE.Focus -> Variable 
                       -> Maybe Hypothesis
createIndHypothesis rel f1 f2 p x = 
    let pattern = PE.toExpr p in
        case pattern of
             (PE.UnOp _ e) -> Just $ createHypothesis "Hipótesis Inductiva" (Expr $ hypIndExpr x e)
                    ...
                    case tType op of
                        t1 :-> t2 :-> t3 -> 
                            case (unifyTest t1 x_type,unifyTest t2 x_type) of
                                 (True,True) -> Just $ createHypothesis "Hipótesis Inductiva" (Expr $ hypIndExprBin x e1 e2)
                                 (True,False) -> Just $ createHypothesis "Hipótesis Inductiva" (Expr $ hypIndExpr x e1)
                                 (False,True) -> Just $ createHypothesis "Hipótesis Inductiva" (Expr $ hypIndExpr x e2)
                                 _ -> Nothing
             _ -> Nothing
